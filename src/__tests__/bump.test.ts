...
import {
  detectConventionalBump,
  detectKeywordBump,
  highestBump,
  detectBumpFromMessages,
} from '../bump';
...
describe('detectConventionalBump', () => {
  ...
  it('detects BREAKING CHANGE footer as major', () => {
    expect(
      detectConventionalBump('feat: something\n\nBREAKING CHANGE: removed api')
    ).toBe('major');
  });
...
describe('detectBumpFromMessages', () => {
  ...
  it('detects from conventional commits', () => {
    const result = detectBumpFromMessages(
      ['feat: new thing', 'fix: old thing'],
      major,
      minor,
      patch
    );
  ...
  it('prefers highest bump across messages', () => {
    const result = detectBumpFromMessages(
      ['fix: small fix', 'feat!: big breaking change'],
      major,
      minor,
      patch
    );
