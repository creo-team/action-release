...
import { formatRawChanges, generateChangelog, parseCommitMessage } from '../changelog'
...
describe('parseCommitMessage', () => {
	...
	it('parses BREAKING CHANGE footer', () => {
		const result = parseCommitMessage('refactor: rewrite\n\nBREAKING CHANGE: dropped support')
...
describe('generateChangelog', () => {
	...
	it('groups scoped commits correctly', () => {
		const result = generateChangelog(['feat(ui): add modal', 'feat(api): add endpoint'])
...
describe('formatRawChanges', () => {
	it('formats commit messages as bullet list', () => {
		const result = formatRawChanges(['first commit', 'second commit'], ['aaa1111', 'bbb2222'])
